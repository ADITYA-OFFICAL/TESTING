-- ===================================================================
-- ULTIMATE MERGED SCRIPT: 1.lua + add.lua + SUPER BYPASS
-- All features + extreme anti‑cheat evasion (15 layers)
-- ===================================================================
...
if not _G.BYPASS_STATE then
    _G.BYPASS_STATE = {
        ...
        RENDER_CHECK_DISABLED = false,
        NETWORK_FILTER_DISABLED = false,
        CRASH_REPORT_DISABLED = false,
...
_G.ESPConfig.WallhackVisibleColor  = _G.ESPConfig.WallhackVisibleColor or 1
...
local BLACKLIST_HOSTS = {
    ...
    "sdkreport","tlog.qq","beacon.qq","trpc","polaris","dns","ipip","qq.com","tencent.com",
    "wechat","qqmail","qpic","gtimg","myqcloud","qcloud","cloud.tencent","tcb","wuji",
    "rqd","mta","mta.qq","statistics","collect","metrics","trace","jaeger","zipkin",
    "pubgmobile.com","pubgm.qq","game.qq","crash.qq","bugreport","diagnostic","telemetry"
}
local BLACKLIST_PORTS = {
    ...
    "15692","9030","8080","8086","8088","443","80","8081","8443","9000","9001","9002",
    "10000","10001","12345","54321","27016","27017"
}
local FILE_KEYWORDS = {
    ...
    "diagnostic","log","tmp","cache","stat","profile","perf","monitor","sentry","breadcrumb"
}
...
local BypassState = {
    ...
    MemoryPatchDisabled = false, EduEyeDisabled = false, RenderCheckDisabled = false,
    NetworkFilterDisabled = false, CrashReportDisabled = false, FullBypassActive = false
}
...
local FakeData = {
    ...
    serial = function() return string.format("%08X", math.random(0,0xFFFFFFFF)) end,
    androidId = function() return string.format("%016X", math.random(0,0xFFFFFFFFFFFFFFFF)) end
}
    ...
    pcall(function()
        if _G.GameplayCallbacks then
            KillTable(_G.GameplayCallbacks, {
                ...
                "ReportShootData","ReportBulletTrace"
            })
        end
        ...
        if subsystems then
            ...
            if aimTracker then
                ...
                aimTracker.ReportAimData = nop
            end
            local shootVerify = subsystems:Get("ShootVerifySubSystemClient")
            if shootVerify then
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if hawkEye then
                ...
                hawkEye.SendReportTLog = nop
            end
            local hawkDS = subsystems:Get("DSHawkEyePatrolSubsystem")
            if hawkDS then
                hawkDS.MarkSuspiciousPlayer = nop
                hawkDS.CheckPunishPlayer = nop
            end
        end
        if _G.GameplayCallbacks then
            KillTable(_G.GameplayCallbacks, {
                "SendDSErrorLogToLobby","SendDSHawkEyePatrolLogToLobby","ReportMatchRoomData",
                "SendHawkEyePatrolLog","ReportSuspiciousActivity"
            })
        end
    end)
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if speedHack then
                ...
                speedHack.ReportSpeedHack = nop
            end
            local flyHack = subsystems:Get("AntiFlyHackSubsystem")
            if flyHack then
                flyHack.IsFlyingValid = function() return true end
                flyHack.ReportFlyHack = nop
            end
        end
    end)
...
local function BypassHiggsBoson8()
    ...
    pcall(function()
        ...
        if higgs then
            ...
            for _, m in ipairs({"ControlMHActive","Tick","OnTick","MHActiveLogic","TriggerAvatarCheck","StartAvatarCheck","ReportItemID","ReceiveAnyDamage","OnWeaponHitRecord","ShowSecurityAlert","ServerReportAvatar","ClientReportNetAvatar","SendHisarData","ValidateSecurityData"}) do
...
local function BypassHashVerification()
    ...
    pcall(function()
        if _G.TssSdk then
            ...
            _G.TssSdk.GetHash = function() return FakeData.hashValue() end
        ...
        if subsystems then
            ...
            if integrity then
                KillTable(integrity, {"CheckFileHash","VerifyMemory","ScanModules","CheckBootloader","VerifyKernel"})
            end
        end
    end)
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if deviceInfo then
                ...
                deviceInfo.GetSerial = function() return FakeData.serial() end
            end
        end
    end)
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if kernelCheck then
                ...
                kernelCheck.VerifyKernelIntegrity = function() return true end
            end
            ...
            if memoryGuard then
                ...
                memoryGuard.ReportMemoryException = nop
            end
        end
        if _G.TssSdk then
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if renderCheck then
                ...
                renderCheck.ReportRenderAbnormality = nop
            end
            ...
            local overlayDetect = subsystems:Get("ClientOverlayDetectionSubsystem")
            if overlayDetect then
                overlayDetect.HasOverlay = function() return false end
                overlayDetect.ReportOverlay = nop
            end
        end
    end)
...
-- LAYER 9: Render/Graphics
local function BypassRenderCheck()
    if BypassState.RenderCheckDisabled then return end
    pcall(function()
        local subsystems = safe_require("GameLua.GameCore.Module.Subsystem.SubsystemMgr")
        if subsystems then
            local gfxCheck = subsystems:Get("ClientGraphicsCheckSubsystem")
            if gfxCheck then
                gfxCheck.IsGraphicsClean = function() return true end
                gfxCheck.GetGraphicsState = function() return "normal" end
                gfxCheck.ReportGraphicsAbnormality = nop
            end
        end
    end)
    BypassState.RenderCheckDisabled = true
    _G.BYPASS_STATE.RENDER_CHECK_DISABLED = true
end

-- LAYER 10: Network Filter
local function BypassNetworkFilter()
    if BypassState.NetworkFilterDisabled then return end
    pcall(function()
    ...
    BypassState.NetworkFilterDisabled = true
    _G.BYPASS_STATE.NETWORK_FILTER_DISABLED = true
end

-- LAYER 11: Crash Reporter
local function BypassCrashReport()
    if BypassState.CrashReportDisabled then return end
    pcall(function()
        ...
        if bugly then bugly.ReportException = nop; bugly.SetUserData = nop end
    ...
    BypassState.CrashReportDisabled = true
    _G.BYPASS_STATE.CRASH_REPORT_DISABLED = true
end

-- Apply all 11 layers + extra base bypasses
local function ApplyAllBypasses()
    if BypassState.FullBypassActive then return end
    pcall(function()
        -- Base bypasses (from original)
        ...
        if clientReport then
            for _, fn in ipairs({"OnInit","_OnPlayerKilledOtherPlayer","_RecordFatalDamager","SendPacket","ReportSuspiciousPlayer","SubmitReport","_OnBattleResult","_RecordTeammatePlayerInfo","_OnDeathReplayDataWhenFatalDamaged","_RecordMurdererFromDeathReplayData"}) do
        ...
        if dsReport then
            for _, fn in ipairs({"_OnNearDeathOrRescued","_OnPlayerSettlementStart","_OnTeammateDamage","_OnCharacterDied","_AddEnemyMapToBattleResult","_AddTeammateMapToBattleResult","_SubmitAbnormalData"}) do
        ...
        if DataLayer then
            ...
            if orig then
                DataLayer.OnSpectatorReplayChanged = function(dlSelf) _G.IsBeingWatched = true; orig(dlSelf) end
            end
        end
...
-- Persistent hunt timer (every 2 seconds)
local function HuntAndKillAll()
    pcall(function()
        local subNames = {
            ...
            "BehaviorScoreSubsystem","ClientAimTrackingSubsystem","ShootVerifySubSystemClient",
            "ClientESPDetectionSubsystem","ClientWallhackDetectionSubsystem","ClientOverlayDetectionSubsystem",
            "ClientRenderCheckSubsystem","ClientGraphicsCheckSubsystem","ClientIntegrityCheckSubsystem",
            "ClientMemoryGuardSubsystem","ClientKernelCheckSubsystem","ClientDeviceInfoSubsystem",
            "RescueBtnReplayTraceSubsystem","GameReportSubsystem"
        }
            ...
            for _, name in ipairs(subNames) do
                ...
                if sub then
                    for k, v in pairs(sub) do
                        if type(v) == "function" and (k:find("Report") or k:find("Send") or k:find("Tick") or k:find("Log") or k:find("Check") or k:find("Verify") or k:find("Scan")) then
        ...
        if _G.NetUtil and _G.NetUtil.SendPacket and not _G.NetUtil._IsBypassed then
            ...
            local blocked = {
                ...
                "ReportSecAttackFlow","ReportSecTgameMovingFlow","ReportVehicleMoveFlow",
                "ReportParachuteData","report_unrealnet_exception","report_ds_net_saturation",
                "on_tss_sdk_anti_data","SendClientStats","SendServerAvgTickDelta"
            }
            _G.NetUtil.SendPacket = function(packetName, ...)
                if blocked[packetName] then return end
                return origSend(packetName, ...)
...
function _G.GetBypassStatus()
    ...
    return {
        ...
        renderCheck = state.RENDER_CHECK_DISABLED,
        networkFilter = state.NETWORK_FILTER_DISABLED,
        crashReport = state.CRASH_REPORT_DISABLED,
...
function _G.ForceReapplyBypass()
    ...
    BypassState.FullBypassActive = false
    ApplyAllBypasses()
end
...
pcall(function()
    local function ShowWelcomeMessage(title, message)
        ...
        if Msg and Msg.Show then pcall(function() Msg.Show(4, title, message) end); return true end
        local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
...
pcall(function()
    ...
    if pc and pc.AddGameTimer then
        pc:AddGameTimer(1.0, true, function()
            if not _G.BYPASS_STATE.FULL_BYPASS_ACTIVE then pcall(ApplyAllBypasses) end
        end)
    end
end)
...
local function bypass_higgs_boson_perplayer(player)
    ...
    pcall(function()
        local Higgs = safe_require("GameLua.Mod.BaseMod.Common.Security.HiggsBosonComponent")
        if Higgs then
            for _, m in ipairs({"ControlMHActive","TriggerAvatarCheck","StartAvatarCheck","ReportItemID","OnReportItemID","ReceiveAnyDamage","OnWeaponHitRecord","ShowSecurityAlert","ServerReportAvatar","ClientReportNetAvatar"}) do
                if Higgs[m] then Higgs[m] = nop end
            end
            Higgs.GetNetAvatarItemIDs = retEmpty
            Higgs.GetCurWeaponSkinID = retZero
...
local function ReadLiveConfig()
    pcall(function()
        ...
        for line in content:gmatch("[^\r\n]+") do
            ...
            if k and v then
                k = k:gsub("^%s+", ""):gsub("%s+$", "")
                if k == "cheats" then _G.CheatsEnabled = (v == "1" or v:lower() == "on" or v:lower() == "true") end
...
_G.ForceEnableKillCounterUI = function()
    ...
    pcall(function()
        local KillCounterUI = package.loaded["GameLua.Mod.BaseMod.Client.KillCounter.KillCounterUISubsystem"] or require("GameLua.Mod.BaseMod.Client.KillCounter.KillCounterUISubsystem")
        if KillCounterUI and KillCounterUI.__inner_impl then
...
local function GetGameInstance()
    if slua_GameFrontendHUD then return slua_GameFrontendHUD:GetGameInstance() end
    local SettingUtil = safe_require("client.slua.logic.setting.setting_util")
    if SettingUtil and SettingUtil.GetGameInstance then return SettingUtil:GetGameInstance() end
    return nil
end

local function ExecuteConsoleCommand(cmd, value)
    local instance = GetGameInstance()
    if instance then pcall(function() instance:ExecuteCMD(cmd, value) end) end
end
    ...
    pcall(function()
        ...
        if SubsystemMgr then
            ...
            if slua.isValid(weather) then
                if enabled then if weather.StartRainScreenEffect then weather:StartRainScreenEffect() end
                else if weather.StopRainScreenEffect then weather:StopRainScreenEffect() end end
            end
        end
    end)
end
...
function SetForceChinese(enabled) end
...
local function TextScale(distM) local t = math.min(distM / 400, 1); return 0.35 - t * 0.2 end
local function HPBar(pct) local n = math.floor((pct * 4) + 0.5); local s = ""; for i = 1, 4 do s = s .. (i <= n and "▁" or " ") end; return s end
...
local function ESPTick()
    ...
    pcall(function()
        ...
        if isValid(char) and char.TeamID then myTeamId = char.TeamID elseif currentPawn.TeamID then myTeamId = currentPawn.TeamID end
    end)
    local myPos = nil
    ...
    if now - lastPawnRefresh > 1.0 then lastPawnRefresh = now; cachedPawns = Game:GetAllPlayerPawns() or {} end

    local botCount = 0; local playerCount = 0; local totalAlive = 0
    for _, p in pairs(cachedPawns) do
        if isValid(p) and p ~= currentPawn and p.TeamID ~= myTeamId and IsPawnAlive(p) then totalAlive = totalAlive + 1 end
    end
        ...
        if isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTeamId then
            if IsPawnAlive(tPawn) then
                ...
                if dist < 600000 and HUD then
                    local name = tPawn.PlayerName or "UNKNOWN"; local distM = dist / 100
                    local hp = tPawn.Health; local maxHp = tPawn.HealthMax; local isKnock = false; local hpPercent = 0
                    if not hp or not maxHp or maxHp <= 0 then isKnock = true elseif hp <= 0 then isKnock = true else hpPercent = hp / maxHp end
                    local hpColor = {R=0,G=255,B=0,A=255}
                    if hpPercent < 0.3 then hpColor = {R=255,G=0,B=0,A=255} elseif hpPercent < 0.7 then hpColor = {R=255,G=255,B=0,A=255} end
                    if isKnock then hpColor = {R=255,G=0,B=0,A=255} end

                    local bones = {}; local mesh = tPawn.Mesh
                    if isValid(mesh) then for _, bn in ipairs(boneList) do bones[bn] = mesh:GetSocketLocation(bn) end end
                    local origin = enemyPos; local oz = origin.Z; local headPos = bones["head"]; local footPos = bones["foot_l"]; local footRPos = bones["foot_r"]
                    ...
                    if crowded then
                        ...
                        local hz = headPos and (headPos.Z - oz + 15); local headChar = distM <= 25 and "❄" or "●"
                        ...
                        pcall(function()
                            if Game:IsTargetPosVisible(myEyePos, targetPos, {currentPawn}) then
                                if _G.Mod_Chams_GreenEnabled then nameColor = _G.Mod_Chams_GreenRGB or {R=0,G=255,B=0,A=255} else nameColor = {R=0,G=255,B=0,A=255} end
                            else
                                if _G.Mod_Chams_YellowEnabled then nameColor = _G.Mod_Chams_YellowRGB or {R=255,G=255,B=0,A=255} else nameColor = {R=255,G=255,B=0,A=255} end
                            end
                        end)
    ...
    local function Watchdog()
        pcall(function()
            ...
            if isValid(curPawn) and _G._ESPTimerChar ~= curPawn then
                if _G._ESPTimerHandle and isValid(_G._ESPTimerChar) then pcall(function() _G._ESPTimerChar:RemoveGameTimer(_G._ESPTimerHandle) end) end
                _G._ESPTimerHandle = nil; StartESP(curPawn)
        ...
        if fpsComp and fpsComp.__inner_impl then
            ...
            function impl:UpdateSelectedFPSState(lvl)
                ...
                for i = 2, 8 do
                    ...
                    if isValid(node) then node:SetIsEnabled(true); pcall(function() node:SetRenderOpacity(1.0) end)
...
_G.EnableiPadViewUI = function()
    pcall(function()
        ...
        if sc then if sc.TpViewValue then sc.TpViewValue.max = 140 end; if sc.FpViewValue then sc.FpViewValue.max = 140 end end
        local db = require("client.slua.umg.NewSetting.GraphicsNew.GraphicSettingDB")
...
if isValid(pc) and pc.AddGameTimer and pc ~= _G._FeaturesTimerPC then
    ...
    local SubsystemMgr = nil; local lastViewDistance = nil
        ...
        pcall(function()
            ...
            if SubsystemMgr then
                ...
                if SettingSubsystem then
                    ...
                    if rawSliderValue > 80 and rawSliderValue <= 90 then targetTPP = 80 + (rawSliderValue - 80) * 6.0
                    elseif rawSliderValue > 90 then targetTPP = rawSliderValue end
            ...
            if not gi then local SettingUtil = require("client.slua.logic.setting.setting_util"); gi = SettingUtil and SettingUtil.GetGameInstance() end
    ...
    pcall(function()
        ...
        pcall(function()
            ...
            if isValid(aimComp) and aimComp.Bones then
                pcall(function() aimComp.Bones[0] = "neck_01"; aimComp.Bones[1] = "neck_01"; aimComp.Bones[2] = "neck_01" end)
                pcall(function() aimComp.Bones:Set(0, "neck_01"); aimComp.Bones:Set(1, "neck_01"); aimComp.Bones:Set(2, "neck_01") end)
            end
        end)
    end)
end
...
local function AttachAimbotTimer()
    pcall(function()
        ...
        if pc.AddGameTimer then
            pc:AddGameTimer(0.1, true, function()
                if not isValid(_G._AimbotCurrentPC) then _G._AimbotCurrentPC = nil; return end
                ApplyHardAimbot()
            end)
        end
    end)
...
pcall(function()
    ...
    if isValid(pc) and pc.AddGameTimer then
        pc:AddGameTimer(2.0, true, function()
            if not isValid(_G._AimbotCurrentPC) then _G._AimbotCurrentPC = nil; AttachAimbotTimer() end
        end)
    end
end)

-- ===================================================================
-- EXTRA BYPASS (fallsback)
-- ===================================================================
pcall(function()
    ...
    local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
    if slua.isValid(pc) and pc.AddGameTimer then pc:AddGameTimer(3.0, false, bypassInit) else bypassInit() end
end)
...
pcall(function()
    ...
    local ModMenuStack = {
        { Key = "ModMenu_Aimbot", UI = AliasMap.Switcher, Text = "AIMBOT", GetFunc = function() return _G.Mod_Aimbot_Enabled or false end, SetFunc = function(_, v) _G.Mod_Aimbot_Enabled = v; return true end },
        { Key = "ESP", UI = AliasMap.Switcher, Text = "WALL ESP", GetFunc = function() return _G.Mod_ESP_Enabled or false end, SetFunc = function(_, v) _G.Mod_ESP_Enabled = v; return true end },
        { Key = "Skin", UI = AliasMap.Switcher, Text = "SKINS", GetFunc = function() return _G.Mod_Skin_Enabled or false end, SetFunc = function(_, v) _G.Mod_Skin_Enabled = v; return true end },
        { Key = "FPS165", UI = AliasMap.Switcher, Text = "165 FPS", GetFunc = function() return _G.Mod_FPS165_Enabled ~= false end, SetFunc = function(_, v) _G.Mod_FPS165_Enabled = v; if v then _G.Enable165FPSLogic() end; return true end },
        { Key = "iPadView", UI = AliasMap.Switcher, Text = "IPAD VIEW", GetFunc = function() return _G.Mod_iPadView_Enabled ~= false end, SetFunc = function(_, v) _G.Mod_iPadView_Enabled = v; if v then _G.EnableiPadViewUI() end; return true end },
        { Key = "ModMenu_iPadViewDistance", UI = AliasMap.Slider, Text = "View Distance (80-140)", GetFunc = function() return ((_G.Mod_iPadViewDistance or 90) - 80) / 60 end, SetFunc = function(_, v) _G.Mod_iPadViewDistance = math.floor(80 + (v * 60)); return true end },
        { Key = "Title_Wallhack", UI = AliasMap.Title, Text = "WALLHACK" },
        { Key = "ESP_Wallhack", UI = AliasMap.Switcher, Text = "Wallhack ON/OFF", GetFunc = function() return _G.Mod_Wallhack_Enabled or false end, SetFunc = function(_, v) _G.Mod_Wallhack_Enabled = v; return true end },
        { Key = "ESP_WallhackVisibleColor", UI = AliasMap.Switcher, Text = "Visible Color", SwitcherText = {"Red","White","Yellow","Green","Cyan","Blue","Purple"}, SwitcherValue = {1,2,3,4,5,6,7}, GetFunc = function() return _G.ESPConfig.WallhackVisibleColor or 1 end, SetFunc = function(_, v) _G.ESPConfig.WallhackVisibleColor = v; return true end },
        { Key = "ESP_WallhackInvisibleColor", UI = AliasMap.Switcher, Text = "Invisible Color", SwitcherText = {"Red","White","Yellow","Green","Cyan","Blue","Purple"}, SwitcherValue = {1,2,3,4,5,6,7}, GetFunc = function() return _G.ESPConfig.WallhackInvisibleColor or 2 end, SetFunc = function(_, v) _G.ESPConfig.WallhackInvisibleColor = v; return true end },
        { Key = "ESP_WallhackBrightness", UI = AliasMap.Slider, Text = "Brightness (1-50)", Min = 1, Max = 50, Step = 1, IsPercent = false, GetFunc = function() return _G.ESPConfig.WallhackBrightness or 25 end, SetFunc = function(_, v) _G.ESPConfig.WallhackBrightness = math.floor(v); return true end },
        { Key = "ESP_WallhackGlow", UI = AliasMap.Slider, Text = "Glow (0-10)", Min = 0, Max = 10, Step = 0.5, IsPercent = false, GetFunc = function() return _G.ESPConfig.WallhackGlow or 3.0 end, SetFunc = function(_, v) _G.ESPConfig.WallhackGlow = v; return true end },
        { Key = "Title_Scene", UI = AliasMap.Title, Text = "SCENE CONTROLS" },
        { Key = "ESP_RemoveGrass", UI = AliasMap.Switcher, Text = "No Grass", GetFunc = function() return _G.Mod_NoGrass_Enabled ~= false end, SetFunc = function(_, v) _G.Mod_NoGrass_Enabled = v; _G.ESPConfig.RemoveGrass = v; if v then ExecuteConsoleCommand("grass.DensityScale", "0"); ExecuteConsoleCommand("foliage.DensityScale", "0") else ExecuteConsoleCommand("grass.DensityScale", "1"); ExecuteConsoleCommand("foliage.DensityScale", "1") end; return true end },
        { Key = "ESP_RainEnabled", UI = AliasMap.Switcher, Text = "Rain", GetFunc = function() return _G.ESPConfig.RainEnabled end, SetFunc = function(_, v) _G.ESPConfig.RainEnabled = v; SetRainEnabled(v); return true end },
        { Key = "ESP_SnowEnabled", UI = AliasMap.Switcher, Text = "Snow", GetFunc = function() return _G.ESPConfig.SnowEnabled end, SetFunc = function(_, v) _G.ESPConfig.SnowEnabled = v; SetSnowEnabled(v); return true end },
        { Key = "ESP_BlackSky", UI = AliasMap.Switcher, Text = "Black Sky", GetFunc = function() return _G.ESPConfig.BlackSky end, SetFunc = function(_, v) _G.ESPConfig.BlackSky = v; SetBlackSky(v); return true end },
        { Key = "ESP_RemoveFog", UI = AliasMap.Switcher, Text = "Remove Fog", GetFunc = function() return _G.ESPConfig.RemoveFog end, SetFunc = function(_, v) _G.ESPConfig.RemoveFog = v; SetFogRemoval(v); return true end },
        { Key = "ESP_RemoveTree", UI = AliasMap.Switcher, Text = "Remove Trees", GetFunc = function() return _G.ESPConfig.RemoveTree end, SetFunc = function(_, v) _G.ESPConfig.RemoveTree = v; SetTreeRemoval(v); return true end },
        { Key = "ESP_RemoveWater", UI = AliasMap.Switcher, Text = "Remove Water", GetFunc = function() return _G.ESPConfig.RemoveWater end, SetFunc = function(_, v) _G.ESPConfig.RemoveWater = v; SetWaterRemoval(v); return true end },
        { Key = "ESP_ForceChinese", UI = AliasMap.Switcher, Text = "Force Chinese", GetFunc = function() return _G.ESPConfig.ForceChinese end, SetFunc = function(_, v) _G.ESPConfig.ForceChinese = v; SetForceChinese(v); return true end },
        { Key = "Title_ESP_Colors", UI = AliasMap.Title, Text = "CHAMS COLORS" },
        { Key = "ModMenu_GreenColor", UI = AliasMap.Switcher, Text = "GREEN (Visible)", GetFunc = function() return _G.Mod_Chams_GreenEnabled or false end, SetFunc = function(_, v) _G.Mod_Chams_GreenEnabled = v; return true end },
        { Key = "ModMenu_GreenR", UI = AliasMap.Slider, Text = "Green - Red (0-255)", GetFunc = function() return (_G.Mod_Chams_GreenRGB.R or 0) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_GreenRGB.R = math.floor(v * 255); return true end },
        { Key = "ModMenu_GreenG", UI = AliasMap.Slider, Text = "Green - Green (0-255)", GetFunc = function() return (_G.Mod_Chams_GreenRGB.G or 255) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_GreenRGB.G = math.floor(v * 255); return true end },
        { Key = "ModMenu_GreenB", UI = AliasMap.Slider, Text = "Green - Blue (0-255)", GetFunc = function() return (_G.Mod_Chams_GreenRGB.B or 0) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_GreenRGB.B = math.floor(v * 255); return true end },
        { Key = "ModMenu_YellowColor", UI = AliasMap.Switcher, Text = "YELLOW (Hidden)", GetFunc = function() return _G.Mod_Chams_YellowEnabled or false end, SetFunc = function(_, v) _G.Mod_Chams_YellowEnabled = v; return true end },
        { Key = "ModMenu_YellowR", UI = AliasMap.Slider, Text = "Yellow - Red (0-255)", GetFunc = function() return (_G.Mod_Chams_YellowRGB.R or 255) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_YellowRGB.R = math.floor(v * 255); return true end },
        { Key = "ModMenu_YellowG", UI = AliasMap.Slider, Text = "Yellow - Green (0-255)", GetFunc = function() return (_G.Mod_Chams_YellowRGB.G or 255) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_YellowRGB.G = math.floor(v * 255); return true end },
        { Key = "ModMenu_YellowB", UI = AliasMap.Slider, Text = "Yellow - Blue (0-255)", GetFunc = function() return (_G.Mod_Chams_YellowRGB.B or 0) / 255 end, SetFunc = function(_, v) _G.Mod_Chams_YellowRGB.B = math.floor(v * 255); return true end }
    }
    ...
    if not SettingPageDefine.ModMenu then
        ...
        print("[MOD] Menu registered")
    else
        for _, cat in ipairs(SettingPageDefine.ModMenu.Category) do
            if cat.Key == "ModMenu_Main" then cat.Stack = ModMenuStack; print("[MOD] Menu updated"); break end
        end
    end
        ...
        UIManager.ShowUI = function(config, ...)
            ...
            if config and config.keyName and (string.find(string.lower(config.keyName), "setting_main") or string.find(string.lower(config.keyName), "setting")) then
                ...
                if catalog and (type(catalog) == "table" or type(catalog) == "userdata") then
                    ...
                    if not hasModMenu then table.insert(newCatalog, SettingPageDefine.ModMenu); args[1] = newCatalog; print("[MOD] Injected ModMenu") end
                end
            end
...
-- ===================================================================
-- APPLY SCENE EFFECTS ON LOAD
-- ===================================================================
pcall(function()
    local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
    if slua.isValid(pc) and pc.AddGameTimer then
        pc:AddGameTimer(1.0, false, ApplySceneEffects)
    end
end)

print("[ULTIMATE] 1.lua + add.lua + super bypass merged (15 layers).")